...
این پروژه با مجوز MIT منتشر شده است. صفحه «درباره پروژه» نیز از مسیر `about.php` در دسترس است.
...
برای ارسال اطلاعات ورود پرتال به مخاطبین، قالب پیامک از تنظیمات قابل تغییر است و از Placeholderهای `{app_title}`، `{contact_name}`، `{customer_name}`، `{email}`، `{password}` و `{portal_url}` پشتیبانی می‌کند.

ارسال ایمیل نیز از تنظیمات پیشرفته قابل فعال‌سازی است و برای اطلاعات ورود پرتال، پاسخ تیکت و یادآوری فعالیت استفاده می‌شود. روش ارسال می‌تواند `PHP mail` یا `SMTP` باشد. برای هاست واقعی، SMTP مطمئن‌تر است و دکمه ارسال ایمیل تست برای بررسی اتصال در نظر گرفته شده است.
...
- مدیریت گزینه‌های Dropdown، واحد پول و کدگذاری خودکار مشتری از تنظیمات
- صفحه راهنمای داخلی برای توضیح فرایند ثبت اطلاعات
